...
import {getUpcomingProfileById} from "../../redux/modules/upcomingProfiles";
...
const ProfileDetailScreen: React.FC<ProfileDetailProps> = ({ route }) => {
    ...
    const profileDetailSelector = useMemo(
        () => (state:any) => getUpcomingProfileById(state, userId),
        [userId]
    );
    const profileData = useSelector(profileDetailSelector);
    useEffect(() => {
        if (userId) {
            setProfileDetail(profileData);
        }
    }, [userId, profileDetailSelector]);



    return (
            !!profileDetail && <SafeAreaView style={{ flex: 1, backgroundColor: 'black' }} edges={['top']}>
        <ScrollView showsVerticalScrollIndicator={false}>
            <View className={" bg-black top-0"} style={{height: screenHeight * (3/5)}}>
                <SwiperFlatList
                    index={0}
                    showPagination
                    data={profileDetail.profileMedia}
                    paginationStyle={{
                    ...
                    renderItem={(item) => {
                        return <View className={"flex-1 f-full w-screen rounded-[16px]"}>
                            <Image source={{uri: item.item.mediaPath}} className={"w-full h-full rounded-[16px]"}/>
                            <LinearGradient
            ...
            <View style={{ backgroundColor: "transparent", marginBottom: 100, padding: 16, display: "flex", flexDirection: "column", gap: 32 }}>
                ...
                <View className={"flex flex-col gap-2"}>
                    <View style={{display: "flex", flexDirection: "row", alignItems: "center", gap:6,}}>
                        <Text style={{ color: "white", fontSize: 30, fontWeight: "bold"}}>{`${profileDetail.userName}, ${calculateAge(profileDetail.dateOfBirth)}`}</Text>
                    ...
                    <View style={{display: "flex", flexDirection: "row", alignItems: "center", gap:6,}}>
                        ...
                        <Text style={{ color: "white", fontSize: 16}}>{`Lives in ${profileDetail.location.city}, ${profileDetail.location.state}, ${profileDetail.location.country}`}</Text>
                    </View>
                </View>
                ...
                <View style={{display: "flex", flexDirection: "row", alignItems: "center", gap:6, backgroundColor: "rgba(91,91,91,0.2)", padding: 16}}>
                    <Icon name={"location-outline"} size={16} color={"#FF7074"}/>
                    <Text style={{ color: "white", fontSize: 16}}>{`~${calculateDistance(profileDetail.location.latitude, profileDetail.location.longitude, userGeoLocationGetter.latitude, userGeoLocationGetter.longitude)} km away, ${profileDetail.location.city}, ${profileDetail.location.state}, ${profileDetail.location.country}`}</Text>
                </View>
